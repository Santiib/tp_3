...
class Grafo(object):
	...
	def camino_minimo(self, origen, destino, camino_mas_largo = False, heuristica=heuristica_nula):
		...
		return self._reconstruir_camino(vuelta, origen, destino)
		
	def _reconstruir_camino(self, vuelta, origen, destino):
		camino = []
		actual = destino
	...
	def _label_mayor_frecuencia(self, vertice, adyacentes, label, v_con_label):
		labels = {}
	...
	def label_propagation(self):
		...
		while not detener:
			v_con_label = 0
			for v in label:
				label_mayor, misma_frecuencia = self._label_mayor_frecuencia(v, self.vertices[v][1], label, v_con_label)
